...
"""
    ...
    command_string = f"pytest {test_files} --doctest-modules --cov='src/panel_vegafusion'"
...
@task()
def pylint(command, files="setup.py tasks src/panel_vegafusion tests"):
...
@task
def mypy(command, files="setup.py tasks src/panel_vegafusion tests"):
